...
class RankingViewController: UIViewController, UITableViewDataSource {
    ...
    private func updateTeamRanking() {
        guard let myIndex = viewModel.myIndex, let needExp = viewModel.needExp else {
            teamRankingLabel.text = "데이터를 불러오는 중입니다."
            rankingDetailLabel.text = nil
            return
        }
    ...
    private func setupSideMenu() {
           // MenuViewController를 루트로 설정
           let menu = MenuViewController()
           sideMenu = SideMenuNavigationController(rootViewController: menu)
           sideMenu?.leftSide = true // 메뉴를 왼쪽에서 열리도록 설정
           sideMenu?.menuWidth = 300 // 메뉴 너비
    ...
    private func setupTeamRankingContainer() {
        ...
        NSLayoutConstraint.activate([
            teamRankingContainer.topAnchor.constraint(equalTo: timeContainer.bottomAnchor, constant: 25), // 기존 timeContainer 아래 배치
