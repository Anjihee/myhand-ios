...
class AdminUserCreateViewController: UIViewController {
    ...
    @objc private func handleCreateUser() {
        ...
        CreateUserService.shared.createUser(request: newUser) { result in
            DispatchQueue.main.async {
                switch result {
                case .success(let response):
                    if response.status == "CREATED" {
                        ...
                        self.showAlert(message: "회원 생성에 실패했습니다.")
                    }
                case .failure(let error):
                    self.showAlert(message: "에러 발생: \(error.localizedDescription)")
                }
            }
        }
    }
